...
# Parámetros Globales
RUTAS_TEST = [
    1001,1002,1003,1004,1005,1006,29000,29001,1050,1051,1007,1008,1009,1010,
    1011,1012,1601,1602,1603,1605,1150,1152,7010,7012,7015,5010,5011,5012,5013,
    6001,6002,6003,6004,6005,1250,1251,1252,1253,1350,1352,1353,1450,1451,
    1001,1002,1003,1004,1005,1006,29000,29001,1050,1051,1007,1008,1009,1010,
    1011,1012,1601,1602,1603,1606,1150,1152,7010,7012,7015,5010,5011,5012,5013,
    6001,6002,6003,6004,6005,1250,1251,1252,1253,1350,1352,1353,1450,1451
]
...
def extraer_datos():
    ...
    query = """select * from comercial_cam.dim_producto where estado='A' and cod_compania = '0070';"""
    ...
    # wr.s3.to_csv(pan_visitas, f's3://aje-analytics-ps-backup/PS_CostaRica/Input/visitas_cam000_{formatted_date}.csv', index=False, boto3_session=my_session)
    ...
    # Filtrar visitas canal 2 y compania 0070
...
def main():
    print("Iniciando Limpieza de Datos (Costa Rica)...")
    comprobar_inputs()
    ...
    ruta_ventas_manana = os.path.join(OUTPUT_DIR, "cr_ventas_manana.parquet")
