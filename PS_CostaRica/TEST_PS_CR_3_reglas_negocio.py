...
S3_PREFIX_OUTPUT = "PS_CostaRica/Output/PS_piloto_v1/"
...
def aplicar_filtros_disponibilidad(pan_rec, df_ventas):
    ...
    skus_val = skus_val[skus_val.cod_compania == 70].copy()
    ...
    # --- 5.-5 Filtro STOCK (D_stock_cr.csv) ---
    stock = wr.s3.read_csv("s3://aje-prd-analytics-artifacts-s3/pedido_sugerido/data-v1/cam/D_stock_cr.csv", boto3_session=my_session)
...
def exportar_resultados(final_rec):
    ...
    # Data para D&A
    s3_path_da = f"s3://{S3_BUCKET_BACKUP}/PS_CostaRica/Output/PS_data_piloto_v1/D_pan_recs_data_{fecha_tomorrow}.csv"
    ...
    print(f"Archivos subidos exitosamente a S3 (D&A, Salesforce).")


def main():
    print("--- INICIANDO REGLAS DE NEGOCIO (Costa Rica) ---")

    # 1. Cargar Data
    ruta_ventas = os.path.join(INPUT_DIR_LIMPIEZA, "cr_ventas_manana.parquet")
