...
S3_PREFIX_OUTPUT = "PS_Guatemala/Output/PS_piloto_v1/"
...
def aplicar_filtros_disponibilidad(pan_rec, df_ventas):
    ...
    skus_val = skus_val[skus_val.cod_compania == 76].copy()
    ...
    # --- 5.-5 Filtro STOCK (D_stock_gt.csv) ---
    stock = wr.s3.read_csv("s3://aje-prd-analytics-artifacts-s3/pedido_sugerido/data-v1/cam/D_stock_gt.csv", boto3_session=my_session)
...
def exportar_resultados(final_rec):
    ...
    s3_path_orders = "s3://aje-prd-pedido-sugerido-orders-s3/PE/pedidos_test/base_pedidos_gt.csv"
...
def main():
    print("--- INICIANDO REGLAS DE NEGOCIO (Guatemala) ---")

    # 1. Cargar Data
    ruta_ventas = os.path.join(INPUT_DIR_LIMPIEZA, "gt_ventas_manana.parquet")
