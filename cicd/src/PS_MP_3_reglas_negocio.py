...
def aplicar_filtros_historia(pan_rec, df_ventas, config):
    ...
    for fecha in last_14_days:
        s3_uri = f"s3://{BUCKET_BACKUP}/{s3_output_prefix}D_base_pedidos_{fecha}.csv"
        try:
...
def exportar_resultados(final_rec, config, pais):
    ...
    # Analytics dataset
    s3_path_da = f"s3://{BUCKET_BACKUP}/{config['s3_output_data_prefix']}D_pan_recs_data_{fecha_tomorrow}.csv"
    ...
    s3_path_sf = f"s3://{BUCKET_BACKUP}/{config['s3_output_prefix']}D_base_pedidos_{fecha_tomorrow}.csv"
