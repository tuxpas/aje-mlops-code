...
# Parámetros Globales
RUTAS_TEST = list(set([
    11101,11102,11103,11104,11105,11201,11202,11203,11204,11205,11206,11207,
    11301,11302,11303,11401,11402,11403,11404,11405,11701,11702,11703,11704,
    11705,11706,11707,11708,11501,11502,11503,11504,11601,11602,11603,11604,
    11801,11802,11803,11901,11902,11101,11102,11103,11104,11105,11201,11202,
    11203,11204,11205,11206,11301,11302,11303,11401,11402,11403,11404,11405,
    11701,11702,11703,11704,11705,11706,11707,11708,11501,11502,11503,11504,
    11601,11602,11603,11604,11801,11802,11803,11901,11902
]))
...
def comprobar_inputs():
    ...
    for objeto in objetos["Contents"]:
        ...
        if key == f"{PREFIX_CAM}ventas_sellout_cam000":
            print(f"OJO: El key {key} no aplica a esta validacion, se omite.")
            continue
        if key.endswith("/"):
            continue
...
def extraer_datos():
    ...
    # 1. Maestro de Productos (Redshift) - compañía 0096
    query = """select * from comercial_cam.dim_producto where estado='A' and cod_compania = '0096';"""
    ...
    # 2. Descargar Visitas - filtrar por PA
    ...
    # wr.s3.to_csv(pan_visitas, f's3://aje-analytics-ps-backup/PS_Panama/Input/visitas_cam000_{formatted_date}.csv', index=False, boto3_session=my_session)
...
def preparar_rutas_y_pesos(df_ventas):
    ...
    # 2. División por Rutas (umbral <= 10 para low SKU)
...
def main():
    print("Iniciando Limpieza de Datos (Panamá)...")
    comprobar_inputs()
    ...
    ruta_ventas_manana = os.path.join(OUTPUT_DIR, "pan_ventas_manana.parquet")
